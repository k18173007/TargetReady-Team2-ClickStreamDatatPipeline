...
import com.target_ready.data.pipeline.exceptions.readFileException
  ...
  def readFile(filePath: String, fileFormat: String)(implicit spark: SparkSession): DataFrame = {
    val readFileData_df: DataFrame =
      try {
        ...
        case e: Exception => {
          readFileException("Unable to read file from the given location: " + filePath)
          spark.emptyDataFrame
        }
      }

    val readFileDataCount: Long = readFileData_df.count()
    if (readFileDataCount == 0)  throw readFileException("Input File is empty: " + filePath)
    readFileData_df
  }
  ...
  def loadDataFromStream(topic: String)(implicit spark: SparkSession): DataFrame = {
    val readFileData_df: DataFrame =
      try {
        ...
        case e: Exception => {
          readFileException("Unable to load data from kafka topic: " + topic)
          spark.emptyDataFrame
        }
      }
    readFileData_df
